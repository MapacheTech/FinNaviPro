...
import { X, Bell, Calendar, Zap, Globe } from 'lucide-react';
...
export const ReminderSettings: React.FC<ReminderSettingsProps> = ({ isOpen, onClose }) => {
  ...
  // Local state for demo purposes. In a real app, this would update the user context/backend.
  const [preferences, setPreferences] = useState(MOCK_USER.notificationPreferences);
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-sm bg-surface border border-white/10 rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-10 fade-in duration-300">
        ...
        <div className="space-y-6">
            ...
            <button 
                onClick={() => {
                    // In real app: saveToBackend(preferences);
                    MOCK_USER.notificationPreferences = preferences; 
                    onClose();
                }}
                className="w-full bg-white text-black font-bold py-4 rounded-2xl hover:scale-[1.02] transition-transform"
            >
                {t('settings.save')}
            </button>
